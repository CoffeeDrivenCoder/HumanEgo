...
class G1HeadRGBDCamera(Camera):
    ...
    def _load_camera_parameters_once(self) -> dict[str, np.ndarray]:
        ...
        if loader_path is None:
            loader_path = Path(__file__).resolve().parents[1] / "G1" / "parameter.py"
        else:
            loader_path = Path(loader_path)
            if not loader_path.is_absolute():
                loader_path = Path(__file__).resolve().parents[1] / loader_path
        if not loader_path.exists():
            fallback_loader = Path(__file__).resolve().parents[1] / "scripts" / "parameters.py"
